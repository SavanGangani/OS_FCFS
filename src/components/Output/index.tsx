...
import { options, columns} from '../Output/GoogleGanttChart';
...
const Output = ({
  ...
  if (!arrivalTime.length || !burstTime.length) {
    ...
    const { solvedProcessesInfo, ganttChartInfo, googleGanttChartInput } = solve(
      arrivalTime,
      burstTime,
    );

    const data = [columns, ...googleGanttChartInput];
    return (
      <StyledOutput>
        <Header>
          <h1>Output</h1>
